...
local function send_data(text, opts)
	opts = opts or {}
	...
	if not opts.raw and not text:match("\n$") then
		text = text .. "\n"
	end
...
function Send.send(text)
	send_text(text)
end

function Send.send_raw(text)
	send_data(text, { raw = true })
end

function Send.send_interrupt()
	Send.send_raw("\003")
end
